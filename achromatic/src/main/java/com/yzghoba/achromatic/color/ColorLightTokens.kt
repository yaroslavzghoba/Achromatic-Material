package com.yzghoba.achromatic.color

import androidx.compose.ui.graphics.Color

internal object ColorLightTokens {
    val Achromatic: Color = PaletteTokens.Achromatic20
    val OnAchromatic: Color = PaletteTokens.Achromatic100
    val AchromaticContainer: Color = PaletteTokens.Achromatic90
    val OnAchromaticContainer: Color = PaletteTokens.Achromatic10
    val Error: Color = PaletteTokens.Error40
    val OnError: Color = PaletteTokens.Error100
    val ErrorContainer: Color = PaletteTokens.Error90
    val OnErrorContainer: Color = PaletteTokens.Error10
    val Background: Color = PaletteTokens.Achromatic98
    val OnBackground: Color = PaletteTokens.Achromatic10
    val Surface: Color = PaletteTokens.Achromatic98
    val SurfaceBright: Color = PaletteTokens.Achromatic98
    val SurfaceDim: Color = PaletteTokens.Achromatic87
    val SurfaceContainerLowest: Color = PaletteTokens.Achromatic100
    val SurfaceContainerLow: Color = PaletteTokens.Achromatic96
    val SurfaceContainer: Color = PaletteTokens.Achromatic94
    val SurfaceContainerHigh: Color = PaletteTokens.Achromatic92
    val SurfaceContainerHighest: Color = PaletteTokens.Achromatic90
    val SurfaceVariant: Color = PaletteTokens.Achromatic90
    val OnSurface: Color = PaletteTokens.Achromatic10
    val OnSurfaceVariant: Color = PaletteTokens.Achromatic30
    val SurfaceTint: Color = Achromatic
    val InverseSurface: Color = PaletteTokens.Achromatic20
    val InverseOnSurface: Color = PaletteTokens.Achromatic95
    val Outline: Color = PaletteTokens.Achromatic50
    val OutlineVariant: Color = PaletteTokens.Achromatic80
    val Scrim: Color = PaletteTokens.Achromatic0
}